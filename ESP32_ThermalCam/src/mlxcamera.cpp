...
void MLXCamera::readPixels()
{
  ...
  MLX90640_CalculateTo(mlx90640Frame1, &mlx90640, SensorEmissivity, tr1, measuredPixels.data());
...
void MLXCamera::drawLegendGraph() const
{
  ...
  int j = 0;
  //commented actual rainbow drawing:
...
void MLXCamera::drawLegendText() const
{
  ...
  tft.setCursor(tft.width() - 25, tft.height() - 14);
  tft.print(String(minTemp).substring(0, 2));
  tft.setCursor(tft.width() - 25, 20);
  tft.print(String(maxTemp).substring(0, 2));
}
...
void MLXCamera::drawCenterMeasurement() const
{
  ...
  const int32_t positionX = tft.width()  - 6; //196; -25 removed
  ...
  tft.setTextSize(6);
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.setCursor(tft.width() - 100, 50);
  tft.print(String(avgCenterTemperature).substring(0, 2));
